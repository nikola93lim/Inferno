...
namespace RPG.Core
{
    public class BlockingObjectsFadeManager : MonoBehaviour
    {
        [SerializeField]
        ...
        private IEnumerator CheckForObjects()
        {
            ...
            while (true)
            {
                ...
                Vector3 positionInFrontOfPlayer = Player.transform.position + Vector3.up * 2f + Player.transform.forward * 2f;

                int hits = Physics.SphereCastNonAlloc(
                    origin,
                    sphereCastRadius, 
                    (positionInFrontOfPlayer - origin).normalized, 
                    Hits, 
                    Vector3.Distance(Camera.transform.position, Player.transform.position + Vector3.up * 2f), 
                    LayerMask);
